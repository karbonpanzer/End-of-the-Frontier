...
namespace EOTF.Core.DecalSystem
{
    public sealed class DialogEditDecals : Window
    {
        ...
        private float DrawSymbolSetup(Rect rect)
        {
            ...
            if (Widgets.ButtonText(randomRect, "EOTF_Decals_RandomSymbol".Translate()) && _symbols.Count > 0)
            {
            ...
            if (_symbols.Count == 0)
            {
                Widgets.Label(row, "EOTF_Decals_NoSymbolsFound".Translate());
                return row.yMax;
            }
            ...
            MakeFloatOptionButtons(row, 
                ...
                _selectedSymbol?.LabelCap ?? "EOTF_Decals_NoSelection".Translate(),
        ...
        private float DrawColorSetup(Rect rect)
        {
            ...
            Text.Font = GameFont.Medium;
            Widgets.Label(header, "EOTF_Decals_Color".Translate());
            Text.Font = GameFont.Small;
        ...
        private void DrawColorButtons(Rect rect, ref Color color)
        {
            ...
            if (Widgets.ButtonText(ideoRect, "EOTF_Decals_SetIdeoColor".Translate()) && hasIdeo) { color = ideoColor; SoundDefOf.Tick_Low.PlayOneShotOnCamera(); }
            GUI.color = Color.white;

            if (Widgets.ButtonText(rndRect, "EOTF_Decals_RandomColor".Translate())) { var p = AllColors(); if (p.Count > 0) { color = p[Rand.Range(0, p.Count)]; SoundDefOf.Tick_Low.PlayOneShotOnCamera(); } }

            if (!hasFav) GUI.color = new Color(1f, 1f, 1f, 0.45f);
            if (Widgets.ButtonText(favRect, "EOTF_Decals_SetFavoriteColor".Translate()) && hasFav) { color = favColor; SoundDefOf.Tick_Low.PlayOneShotOnCamera(); }
            GUI.color = Color.white;
        }
        ...
        private void DrawBottomButtons(Rect rect)
        {
            ...
            if (Widgets.ButtonText(new Rect(rect.x, y, w, 28f), "EOTF_Decals_Reset".Translate())) { _profile = _original; _selectedIndex = FindSymbolIndex(_profile.SymbolPath); SyncSelection(); PushLive(); }
            if (Widgets.ButtonText(new Rect(rect.x + w + 10f, y, w, 28f), "EOTF_Decals_Apply".Translate())) { _committed = true; Close(); }
